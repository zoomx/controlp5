...
/**
 * @shortdesc a textarea can be used to leave notes inside of controlP5 displayed on the screen. 
 * 
 * scrollbars are available when text extends the visible area. Textarea
 * extends ControllerGroup, for more available methods see the ControllerGroup
 * documentation.
 * 
...
public class Textarea extends ControllerGroup implements ControlListener {
	...
	protected void preDraw(
		...
		if (_myScrollbar.isVisible() || isColorBackground) {
			...
			theApplet.rect(0, 0, _myWidth + 1 + _myValueLabel.style().paddingLeft + _myValueLabel.style().paddingRight,
			        _myHeight - 1 + _myValueLabel.style().paddingTop + _myValueLabel.style().paddingBottom);
			theApplet.noStroke();
		}
	}
