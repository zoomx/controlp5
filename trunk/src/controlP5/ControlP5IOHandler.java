...
public class ControlP5IOHandler {
	 ...
	 * @return String
	 */
	public static String forHTMLTag(
	        String aTagFragment) {
	...
	/**
	 * http://processing.org/discourse/yabb_beta/YaBB.cgi?board=Programs;action=display;num=1159828167;start=0#0
	 * 
	 * @param string
	 *            String
	 * @return String
	...
	public static String replace(
	        ...
	        String theSearchForString,
	        String theReplaceString) {
	...
	protected boolean save(
		...
		myRoot.setAttribute("lock", "" + theControlP5.isLock);
		...
		for (int k = 0; k < theControlP5.controlWindows().size(); k++) {
			...
			if (!myColor.equals(myControlWindow.color)) {
				...
				myPappletXMLElement.setAttribute("colorBackground", ControlP5IOHandler
				        .intToString(myColor.colorBackground));
				myPappletXMLElement.setAttribute("colorForeground", ControlP5IOHandler
				        .intToString(myColor.colorForeground));
			...
			for (int j = 0; j < myTabs.size(); j++) {
				...
				if (!myColor.equals(myTab.color)) {
					...
					myTabXMLElement.setAttribute("colorBackground", ControlP5IOHandler
					        .intToString(myColor.colorBackground));
					myTabXMLElement.setAttribute("colorForeground", ControlP5IOHandler
					        .intToString(myColor.colorForeground));
		...
		if (theControlP5.urlPath().toLowerCase().startsWith("http://")
			...
			try {
				...
				out.write("controlP5="
				        + ControlP5IOHandler.forURL(myHeader
				...
				System.out.println("### ERROR ###\n" + "### Error while uploading xml to " + theControlP5.urlPath()
				        + " " + e);
			}
		}
		// save xml to file.
		try {
			if (!theControlP5.papplet.online) {
				if (theFilePath.length() > 1) {
					...
					theControlP5.papplet.saveStrings(theFilePath, new String[] { myHeader,
					        ControlP5IOHandler.replace(myRoot.toString(), ">", ">\n") });
			...
			System.out.println("### WARNING ###\n" + "### did not write to file " + theFilePath
	...
	protected boolean parse(
		...
		try {
			...
			Vector myControlWindows = myXML.getChildren();
			...
			for (int a = 0; a < myControlWindows.size(); a++) {
				ControlP5XMLElement myWindowXMLElement = (ControlP5XMLElement) myControlWindows.get(a);
				...
				if (myWindowXMLElement.getStringAttribute("colorBackground") != null) {
					...
					myColor.colorActive = ControlP5IOHandler.parseHex(myWindowXMLElement
					        .getStringAttribute("colorActive"));
					myColor.colorLabel = ControlP5IOHandler.parseHex(myWindowXMLElement
					        .getStringAttribute("colorLabel"));
					myColor.colorValue = ControlP5IOHandler.parseHex(myWindowXMLElement
					        .getStringAttribute("colorValue"));
				}
				if ((myWindowXMLElement.getStringAttribute("class")).startsWith("controlP5.PAppletWindow")) {
					myControlWindow = controlP5.addControlWindow(myWindowXMLElement.getStringAttribute("name"),
					        myWindowXMLElement.getIntAttribute("x"), myWindowXMLElement.getIntAttribute("y"),
					        myWindowXMLElement.getIntAttribute("width"), myWindowXMLElement.getIntAttribute("height"));
					myControlWindow.setBackground(ControlP5IOHandler.parseHex(myWindowXMLElement
					        .getStringAttribute("background")));
				} else {
				...
				Vector myTabs = myWindowXMLElement.getChildren();
				/**
				 * @todo
				...
				for (int i = 0; i < myTabs.size(); i++) {
					ControlP5XMLElement myTabXMLElement = (ControlP5XMLElement) myTabs.get(i);
					...
					Vector myControllers = myTabXMLElement.getChildren();
					ControlGroup myGroup = null;

					for (int j = 0; j < myControllers.size(); j++) {
						ControlP5XMLElement myControllerElement = (ControlP5XMLElement) myControllers.get(j);
	...
	private void parseGroup(
		...
		Vector myChilds = theControllerElement.getChildren();
		for (int j = 0; j < myChilds.size(); j++) {
			ControlP5XMLElement myControllerElement = (ControlP5XMLElement) myChilds.get(j);
			if (myControllerElement.getName().equals("controller")) {
	...
	private void parseController(
		...
		if (theControllerElement.getStringAttribute("type").equals("slider")) {
			myController = controlP5.addSlider(theControllerElement.getStringAttribute("name"),
			        (float) theControllerElement.getDoubleAttribute("min"), (float) theControllerElement
			                .getDoubleAttribute("max"), (float) theControllerElement.getDoubleAttribute("value"),
			        (int) theControllerElement.getDoubleAttribute("x"), (int) theControllerElement
			                .getDoubleAttribute("y"), theControllerElement.getIntAttribute("width"),
		...
		/* knob */
			...
			myController = controlP5.addKnob(theControllerElement.getStringAttribute("name"),
			        (float) theControllerElement.getDoubleAttribute("min"), (float) theControllerElement
			                .getDoubleAttribute("max"), (float) theControllerElement.getDoubleAttribute("value"),
			        (int) theControllerElement.getDoubleAttribute("x"), (int) theControllerElement
			                .getDoubleAttribute("y"), theControllerElement.getIntAttribute("width"));
		...
		/* bang */
			...
			myController = controlP5.addBang(theControllerElement.getStringAttribute("name"),
			        (int) theControllerElement.getDoubleAttribute("x"), (int) theControllerElement
			                .getDoubleAttribute("y"), (int) theControllerElement.getDoubleAttribute("width"),
		...
		/* radio */
		      ...
		      Radio myRadio = controlP5.addRadio(theControllerElement.getStringAttribute("name"),
		                      (int) theControllerElement.getDoubleAttribute("x"),
		                      (int) theControllerElement.getDoubleAttribute("y")
		                      );
		      Vector myRadioButtons = theControllerElement.getChildren();
		      for (int k = 0; k < myRadioButtons.size(); k++) {
		        ControlP5XMLElement myRadioButtonElement = (ControlP5XMLElement) myRadioButtons.get(k);
		...
		/* ScrollList */
			...
			ScrollList myScrollList = controlP5.addScrollList(theControllerElement.getStringAttribute("name"),
			        (int) theControllerElement.getDoubleAttribute("x"), (int) theControllerElement
			                .getDoubleAttribute("y"), (int) theControllerElement.getDoubleAttribute("width"),
			        (int) theControllerElement.getDoubleAttribute("height"));
			Vector myScrollListButtons = theControllerElement.getChildren();
			for (int k = 0; k < myScrollListButtons.size(); k++) {
				ControlP5XMLElement myRadioButtonElement = (ControlP5XMLElement) myScrollListButtons.get(k);
				myScrollList.addItem(myRadioButtonElement.getStringAttribute("name"),
				        (int) myRadioButtonElement.getDoubleAttribute("value")).setId(
				        myRadioButtonElement.getIntAttribute("id"));
			}
		...
		/* toggle */
			...
			myController = controlP5.addToggle(theControllerElement.getStringAttribute("name"),
		...
		/* textfield */
			...
			myController = controlP5.addTextfield(theControllerElement.getStringAttribute("name"),
			        (int) theControllerElement.getDoubleAttribute("x"), (int) theControllerElement
			                .getDoubleAttribute("y"), theControllerElement.getIntAttribute("width"),
			        theControllerElement.getIntAttribute("height"));
			int myId = (int) theControllerElement.getIntAttribute("id");
		...
		/* numberbox */
			...
			myController = controlP5.addNumberbox(theControllerElement.getStringAttribute("name"),
			        (float) theControllerElement.getDoubleAttribute("value"), (int) theControllerElement
			                .getDoubleAttribute("x"), (int) theControllerElement.getDoubleAttribute("y"),
			        (int) theControllerElement.getDoubleAttribute("width"), (int) theControllerElement
		...
		/* button */
			...
			myController = controlP5.addButton(theControllerElement.getStringAttribute("name"),
			        (float) theControllerElement.getDoubleAttribute("value"), (int) theControllerElement
			                .getDoubleAttribute("x"), (int) theControllerElement.getDoubleAttribute("y"),
			        (int) theControllerElement.getDoubleAttribute("width"), (int) theControllerElement
			                .getDoubleAttribute("height"));
	...
	private CColor checkColor(
	        ControlP5XMLElement theControllerElement,
	        CColor theColor) {
		if (theControllerElement.getStringAttribute("colorBackground") != null) {
			theColor.colorBackground = ControlP5IOHandler.parseHex(theControllerElement
			        .getStringAttribute("colorBackground"));
			theColor.colorForeground = ControlP5IOHandler.parseHex(theControllerElement
			        .getStringAttribute("colorForeground"));
